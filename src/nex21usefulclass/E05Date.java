...
public class E05Date {
	...
	public static String myDateFormat(Calendar c, 
		...
		까지의 시간을 초 단위로 반환해준다. 이를 타임스템프라고 
		한다.*/
		Date date = c.getTime(); 
	...
	public static void main(String[] args) {
		...
		/*
		Calendar 클래스는 대표적인 싱글톤 객체이므로 new로 인스턴
		스를 생성할 수 없고, 유틸리티 메서드를 통해 참조값을 얻어온
		다. */
		Calendar calendar = Calendar.getInstance();
		
		/* 월을 표현하는 Month, 분을 표현하는 Minute의 이니셜이
		동일하기 때문에 구분을 위해 월은 대문자, 분은 소문자를 사용
		한다. */
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			...
			서로 다르다. 따라서 각 날짜의 서식을 명확히 지정해야지
			만 날짜를 인식하여 연산할 수 있다. 
	        ...
	        /* getTime()으로 얻어오는 시간은 13자리의 정수값인
	        데 10자리까지는 순수한 초(Sec)이고, 11~13자리는 
